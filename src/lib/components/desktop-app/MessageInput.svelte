<script lang="ts">
	...
	onMount(async () => {
		console.log('Mounting MessageInput');
		loaded = true;
					...
					<Commands
						...
						on:select={(e) => {
							...
							if (data?.type === 'model') {
								...
								console.log('IS THIS WHERE MODEL SELECTION HAPPENS?', data);
							}
							...
							>
								...
								<div class="flex" data-tauri-drag-region>
									<div class="ml-1 self-end mb-1.5 flex space-x-1">
										...
										>
											<button
												class="ml-[-0.25rem] bg-transparent hover:bg-white/80 text-gray-800 dark:text-white dark:hover:bg-gray-800 transition rounded-full p-2 outline-none focus:outline-none"
												type="button"
												aria-label="Options"
											>
